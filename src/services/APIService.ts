...
class APIService {
  ...
  constructor({ baseUrl = "https://api.example.com/v1", headers = {} }: APIServiceConfig = {}) {
  ...
  // GET request
  async get<T>(endpoint: string, config: object = {}, headers?: Record<string, string>): Promise<T> {
  ...
  // DELETE request
  async delete<T>(endpoint: string, config: object = {}, headers?: Record<string, string>): Promise<T> {
